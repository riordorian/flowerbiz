	...
	public function sendEverydaySms($arParams = [])
	{
		...
		if( $arSmsSettings['ACTIVE'] == 1 && !empty($arSmsSettings['API_ID']) ){
			$arEvents = ClientsEvents::find()
				...
				->select(['PHONE' => 'clients.PHONE', 'BONUS' => 'clients.BONUS', 'CLIENT_NAME' => 'clients.NAME', 'EVENT' => 'events.NAME',  'RECIPIENT' => 'gift_recipients.NAME'])
			...
			if( !empty($arEvents) ){
				...
				foreach($arEvents as $arEvent){
					...
					$obData->multi[$arEvent['PHONE']] = 'Добрый день, ' . $arEvent['CLIENT_NAME'] . '! :) Завтра у Вас знаменательное событие, начисляем 200 б. до ' . date('d.m.Y', strtotime('+1 week')) . '. Баланс - ' . $arEvents['BONUS'] . '. Ждем Вас за подарком.';
				}
